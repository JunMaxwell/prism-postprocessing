...
function Scene() {
	...
	const rayMove = useCallback((event: ReflectEvent) => {
		const { api, position, direction, normal } = event;
		if (!normal || !direction) return;
		if (!rainbow.current || !flare.current || !spot.current) return;
	...
	return (
		<>
			...
			<Center top bottom position={[0, 2, 0]}>
				<Text3D size={0.7} letterSpacing={-0.05} height={0.05} font={`${import.meta.env.VITE_PUBLIC_URL}/fonts/Inter_Bold.json`}>
